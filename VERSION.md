...
- Repositório: [público no GitHub](https://github.com/pedrofavarobelini/veltrix),
  com branch padrão `main`.
- Aceite visual final: **`HUMAN_VISUAL_ACCEPTANCE = PASS`**.
- Project Registry: concluído, com persistência InMemory, LocalJson e PostgreSQL.
- Migrations atuais: **`0001`–`0012`**, todas aditivas; `0012` pertence ao
  Project Registry.
- Licença: Apache-2.0 presente em `LICENSE`, `apps/api/pyproject.toml` e
  `apps/web/package.json`.

## Checkpoint documental das Eras 1–3 — histórico
...
## Checkpoint histórico — Control Plane completo (Eras 1 a 10)
...
## Checkpoint histórico — PEDROCORE-V1-FINAL-CLOSURE
...
Naquele checkpoint, o projeto foi **finalizado localmente**:
`PEDROCORE-IMPLEMENT-05` (05A–05F, integrações reais controladas) e
`PEDROCORE-FINALIZE-06` (06A enforcement final + 06B fechamento) concluídas,
com o marco técnico `v7.0.0`. Ver
`Veltrix/13-fechamento/FECHAMENTO_PEDROCORE_FINAL.md`.
...
`v7.0.0` é o marco técnico do core operacional seguro. No histórico público
sanitizado, a tag anotada aponta para
`6c4757a79ccb0b032eb860647efb1d345815d3b8`.

`v6.0.0` existe e aponta para
`502acb8ee02ac6a7c007c823b73a1ad745836118`. A tag representa o fechamento do
MVP backend; ela não acompanha a versão de produto.

Não há GitHub Releases publicadas. As tags preservam marcos históricos com
taxonomia diferente da versão de produto e da versão técnica da API; por isso,
nenhuma Release nova deve ser inferida apenas a partir do número da tag.
...
- **Concluída em 03/09/2026.** O repositório é público e contém `LICENSE` sob
  Apache-2.0. Isso não significa que a API esteja pronta para exposição na
  internet.
