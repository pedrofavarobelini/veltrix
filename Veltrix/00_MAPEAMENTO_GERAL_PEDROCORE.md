...
Links centrais: [[MOC_VELTRIX]] | [[MOC_MULTI_PROVIDER_SAFE_EVOLUTION]] | [[MOC_ARQUITETURA]] | [[MOC_SEGURANCA]] | [[MOC_QA_RELEASE_GATE]] | [[MOC_QA_SAFETY_HARDENING]] | [[MOC_INTEGRACOES]] | [[MOC_TESTES]] | [[MOC_VERSOES_STATUS]] | [[MOC_ESTUDO_PEDROCORE]]

## 1. Visao geral

O Veltrix esta finalizado localmente como core operacional seguro. O estado tecnico local verificado nesta tarefa e:
...
- Resposta esperada: `status="ok"`, `service="Veltrix"`, `version="0.2.0"`.
...
```json
{
  "message": "Explique o Veltrix em uma frase.",
  "provider": "mock",
  "mode": "tecnico"
}
```
...
- Auth: se `PEDROCORE_INTERNAL_API_KEY` estiver definido no ambiente, exige header `X-Veltrix-Api-Key`; se nao estiver, opera em modo dev/local com warning `INTERNAL_AUTH_NOT_CONFIGURED`.
...
Exploration e assistida/manual. O Veltrix gera plano, passos manuais, riscos, evidencias exigidas e confirmacoes humanas. `can_execute_actions` e sempre `false`.
...
## 20. Integracao com FinGuard pelo lado Veltrix
...
- provider/modelo final é decidido pelo Veltrix;
...
- Contrato FinGuard pelo lado Veltrix.
- Artifact Reader opt-in.
...
- Orçamento de saída (`min(global_cap, model_cap, task_cap)`) e timeout de transporte são decididos só pelo Veltrix; o consumidor não tem campo para influenciá-los. Os valores derivam do `response_style` das tasks, não de medição real de tokens.
...
- Documentos historicos ainda existem e podem conter contexto de fase antiga; use [[MOC_VELTRIX]] e este documento como entrada atual.
