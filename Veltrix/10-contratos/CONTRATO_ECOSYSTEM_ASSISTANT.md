...
O Veltrix é o **serviço central de IA do ecossistema**. Sistemas consumidores (FinGuard, AutoIntel, futuros projetos) consomem IA **através do backend deles → `POST /api/orchestrate`**, nunca integrando providers diretamente.
...
Front-end do sistema consumidor
  ...
  -> Veltrix POST /api/orchestrate (com X-Veltrix-Api-Key quando configurada)
...
- O FinGuard já possui estrutura de assistente/tela/botão no repositório próprio; a integração do Assistente FinGuard via Veltrix pertence à frente `FINGUARD-PEDROCORE-ASSISTANT-01` e **não foi implementada nesta frente**.
- O Veltrix não lê, não escreve e não executa nada no FinGuard; artefatos chegam por payload.
...
- Autenticação interna opcional: `PEDROCORE_INTERNAL_API_KEY` + header `X-Veltrix-Api-Key` (mesma regra para as rotas de memória técnica).
