# Veltrix — Contratos Técnicos (Índice)

> Nota DOCFIX: este documento nasceu como planejamento da frente `PEDROCORE-REPLAN-01B`. Em `v7.0.0`, parte relevante desses contratos já foi implementada no lado Veltrix. Use [[../00_MAPEAMENTO_GERAL_PEDROCORE]] como mapa atual e leia este arquivo como histórico/contrato de origem.

## Objetivo

Documentar, antes de qualquer implementação, os contratos técnicos que vão guiar a evolução do Veltrix de "API de chat multi-provider" para **orquestrador central de IA do ecossistema Pedro**, consumível por sistemas externos.
...
| Contrato de orquestração (`origin_system`, `task_type`, `context`, `artifacts`) | **Implementado no lado Veltrix** em `/api/orchestrate`; cliente externo ainda é frente separada. |
...
| Integração real com o FinGuard | **Pronta no lado Veltrix por contrato HTTP**; cliente no repositório FinGuard ainda não implementado aqui. |
...
- O FinGuard é projeto externo; o Veltrix não altera, não roda migrations, não roda seed/reset e não comita nele.
...
- O lado Veltrix implementa exploração assistida/manual como plano, sem execução autônoma — detalhado em `CONTRATO_QA_INTELLIGENCE.md` e no mapa geral.
