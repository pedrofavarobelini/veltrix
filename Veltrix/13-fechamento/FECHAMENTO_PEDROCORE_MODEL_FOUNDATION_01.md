...
Preparar o Veltrix **arquiteturalmente** para evoluir de orquestrador multi-provider para núcleo de inteligência própria do ecossistema, com fundação segura para Intelligence Layer, Report Intelligence, memória técnica futura, avaliação de respostas, contrato de Local Model Provider e documentação honesta dos limites.

Entrega correta desta frente: *"Veltrix preparado arquiteturalmente para evoluir para um núcleo de inteligência própria"* — **não** *"Veltrix agora é um modelo de IA próprio"*.

## 2. Estado inicial verificado

- Branch: `main`; HEAD: `d23d18d` (docs: sanear documentacao Veltrix v7).
...
- Sem leitura do FinGuard. O FinGuard já possui estrutura de assistente/tela/botão no repositório próprio, mas a integração do Assistente FinGuard via Veltrix ainda não foi implementada nesta frente. Essa integração pertence à frente `FINGUARD-PEDROCORE-ASSISTANT-01`.
...
2. `PEDROCORE-ECOSYSTEM-FINALIZE-01` — consolidação do Veltrix como serviço do ecossistema.
3. `FINGUARD-PEDROCORE-ASSISTANT-01` — integração do Assistente FinGuard via Veltrix. O FinGuard já possui estrutura de assistente/tela/botão no repositório próprio; esta frente conecta essa estrutura ao Veltrix (frente própria, lado cliente fora deste repositório).
...
```text
feat: preparar fundacao de inteligencia propria do Veltrix
```
