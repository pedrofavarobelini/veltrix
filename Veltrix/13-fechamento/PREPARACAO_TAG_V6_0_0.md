# Registro pós-tag v6.0.0 — Veltrix
...
- Mensagem da tag: `v6.0.0 - MVP backend Veltrix`.
...
- **Auth interna opcional** (`orchestration/router.py`): `PEDROCORE_INTERNAL_API_KEY` + header `X-Veltrix-Api-Key`, exclusiva de `/api/orchestrate`.
...
- [x] Mensagem da tag: `v6.0.0 - MVP backend Veltrix`.
...
```powershell
git tag -a v6.0.0 -m "v6.0.0 - MVP backend Veltrix"
```
