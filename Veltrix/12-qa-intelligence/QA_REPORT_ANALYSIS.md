...
> Nota DOCFIX: este caso de uso foi implementado no lado Veltrix como heurística textual local em `POST /api/orchestrate` e `/api/chat`, usando `task_type="qa_report_analysis"`. Não é IA real, não lê o FinGuard e não executa testes.
...
- Relatórios incompletos ou truncados devem resultar em `confidence` baixo, nunca em suposições preenchidas pelo Veltrix para "completar" a análise.
