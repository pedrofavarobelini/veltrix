...
> Nota DOCFIX: este caso de uso foi implementado no lado Veltrix em `POST /api/orchestrate` e `/api/chat`, usando `task_type="release_gate_review"`. O release gate é conservador e só aprova com `local_qa` + evidência textual limpa; não executa merge, deploy, tag ou teste.
...
**O Veltrix não decide sozinho. O Veltrix recomenda; o usuário/desenvolvedor aprova.** Nenhum merge, deploy ou tag é criado ou disparado automaticamente a partir desta resposta — a ação de release permanece sempre uma decisão humana, executada fora do Veltrix.
...
- O Veltrix nunca executa a ação de "avançar" (merge, deploy, tag) — apenas recomenda textualmente através da resposta estruturada.
