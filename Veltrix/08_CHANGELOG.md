...
- Project Context `elyra` e Caller Identity registrada `common_consumer`, sem
  reutilizar identidades Veltrix/FinGuard/Structa.
...
> Essa era a raiz canônica do vault na época; hoje ela é `Veltrix/…`, com
...
> troque o prefixo `docs/` por `Veltrix/`.
...
- Cinco arquivos locais `Veltrix/.obsidian/` preservados byte a byte e
...
- A raiz canônica do vault passou de `docs/` para `Veltrix/`.
...
Status: **Veltrix encerrado — core operacional concluído. Assistente IA
...
- `_generate_with_timeout()` deixa de repassar `payload.model`: o adapter recebe apenas o modelo já validado pelo Veltrix. Defaults do adapter viram seleção explícita do core (`model_source=provider_default`).
...
- Matriz `project_id + caller_role + environment + provider` com default **negar**; somente Gemini está registrado (FinGuard em todos os ambientes, Veltrix técnico fora de produção). Negar vira fallback Mock seguro, nunca segundo provider real.
...
- Replay local FinGuard → Veltrix → FinGuard aprovado em sucesso, relatório/memória, fallback e falha total, com cleanup.
...
- Veltrix local subido com `.venv\Scripts\python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 3333` (nunca `python -m uvicorn` generico) e `GEMINI_API_KEY` carregada só via `.env` local (nunca impressa, nunca exportada manualmente).
...
- Com `allow_real_provider=true`, `provider=auto` escolhe Gemini quando `GEMINI_API_KEY` esta configurada no ambiente Veltrix; `provider=gemini` continua exigindo autorizacao explicita.
...
- MOCs centrais: `MOC_VELTRIX`, `MOC_QA_RELEASE_GATE`, `MOC_TESTES`, `MOC_SEGURANCA`, `MOC_VERSOES_STATUS`.
...
- Veltrix continua sendo orquestrador central, nao modelo treinado.
...
Após o fechamento local `v7.0.0`, alguns documentos históricos ainda descreviam como "futuro" ou "não implementado" recursos já existentes no lado Veltrix, como `/api/orchestrate`, Task Router, Project Context, Prompt Builder, QA textual local, Artifact Reader opt-in, policy enforcement forte e integração FinGuard controlada.
...
- Criados MOCs Obsidian: `MOC_VELTRIX`, `MOC_ARQUITETURA`, `MOC_SEGURANCA`, `MOC_QA_RELEASE_GATE`, `MOC_INTEGRACOES`, `MOC_TESTES`, `MOC_VERSOES_STATUS`.
...
### Bloco 8 — Contrato FinGuard → Veltrix (payload fake)

- `project_context/service.py` — `finguard-local` adicionado; `allowed_tasks` do FinGuard e do Veltrix ganharam as tasks exploratórias; notas atualizadas; novo conjunto `FINGUARD_ORIGIN_SYSTEMS`.
...
- Mensagem da tag registrada: `v6.0.0 - MVP backend Veltrix`.
...
Evoluir o Veltrix de "QA skeleton sem análise real" para um MVP operacional: análise textual QA local e determinística, release gate conservador, endpoint operacional `/api/orchestrate`, safe mode de providers reais, autenticação interna opcional, contrato padronizado de warnings/errors e audit não persistente completo.
...
- `POST /api/orchestrate` com auth interna opcional: se `PEDROCORE_INTERNAL_API_KEY` configurada, exige header `X-Veltrix-Api-Key` (ausente → 401 `INTERNAL_AUTH_MISSING`; errado → 401 `INTERNAL_AUTH_INVALID`); sem chave configurada, modo dev/local com warning `INTERNAL_AUTH_NOT_CONFIGURED`. A chave nunca é retornada. `/api/chat` continua livre.
...
Integração real com FinGuard, leitura real de arquivos, execução de comandos pelo Veltrix, QA visual real, OCR, Playwright, agente exploratório, dashboard, log persistente, provider real liberado em fluxo crítico, Blocos 7–11 do planejamento maior, Bloco 12 e Blocos 13–15 finais.
...
Com a arquitetura-alvo documentada em `PEDROCORE-REPLAN-01C` (commit `c1e7816`), a frente `01D` documenta especificamente a camada futura de QA Intelligence: como o Veltrix poderia apoiar sistemas externos, especialmente o QA Automation do FinGuard, na análise inteligente de relatórios, logs, evidências e falhas, e na recomendação assistida de avanço/bloqueio de release.
...
Com a visão oficial consolidada em `PEDROCORE-REPLAN-01A` (commit `1e5a8cb`), a frente `01B` documenta os contratos técnicos que guiarão a evolução do Veltrix como orquestrador central: como sistemas externos poderiam futuramente enviar mensagem/contexto/tipo de tarefa e como o Veltrix devolveria uma resposta padronizada, incluindo o caso específico de análise de QA.
...
Uma auditoria somente leitura do repositório apontou duplicidade documental significativa (pares de arquivos conflitantes em `docs/`), uma visão de projeto desatualizada (Veltrix descrito apenas como chat pessoal multi-provider) e a necessidade de reposicionar o projeto como orquestrador central de IA do ecossistema Pedro, incluindo apoio futuro a inteligência operacional/QA de projetos externos como o FinGuard.

### Alterado (documentação)

- `README.md` — reescrito para apresentar o Veltrix como orquestrador central de IA, multi-provider, API para sistemas externos.
...
- Loading visual `Veltrix está pensando...`.
...
- Mantido topo principal com logo + Veltrix.
