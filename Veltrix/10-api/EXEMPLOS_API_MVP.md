...
POST /api/chat
{
  "message": "Explique o que é o Veltrix",
  "mode": "tecnico",
  "provider": "mock"
}
```
...
Comportamento esperado: o Veltrix **nunca lê o arquivo indicado**. O artefato é rejeitado:
...
- Se `PEDROCORE_INTERNAL_API_KEY` **estiver configurada**: toda chamada a `/api/orchestrate` exige o header `X-Veltrix-Api-Key` com o valor correto.
...
```
POST /api/orchestrate
Header: X-Veltrix-Api-Key: <valor-configurado-no-ambiente>
```

A chave em si nunca é retornada em nenhuma resposta da API.

## 6. Payload fake FinGuard → Veltrix (Bloco 8)
