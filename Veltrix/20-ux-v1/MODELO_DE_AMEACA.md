...
O Veltrix é seguro **para os usos que ele realmente tem hoje**, e não é
...
| **B** | Ecossistema local (FinGuard, Structa e outros consumindo o Veltrix) | **SEGURO** |
...
- Sem XSS: nenhum `dangerouslySetInnerHTML`, `innerHTML` ou `eval` em
  `apps/web/src`.
- Áudio do microfone não é gravado, guardado nem enviado ao Veltrix.
...
de implementação — o Veltrix nunca foi construído para esse cenário.
