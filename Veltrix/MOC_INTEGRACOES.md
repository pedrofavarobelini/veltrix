...
Mapa de integracoes controladas do lado Veltrix.
...
- Veltrix recebe somente snapshot explícito; não acessa banco/Storage Elyra.
...
- Structa não herda identidade ou permissões FinGuard/Veltrix.
...
- O Veltrix reconhece `origin_system=finguard` e `origin_system=finguard-local`.
- O lado Veltrix esta pronto para receber payload HTTP em `/api/orchestrate`.
...
- O cliente comum pede `provider=auto` sem modelo; identidade, autorização, provider e modelo são decididos pelo Veltrix.
- O Veltrix nao acessa, le, escreve, testa ou comita no FinGuard.
- Artifact Reader e bloqueado para origem/caminho FinGuard.
- Provider real é bloqueado por padrão; execução exige `allow_real_provider=true`, identidade registrada, credencial própria no Veltrix e binding elegível.
