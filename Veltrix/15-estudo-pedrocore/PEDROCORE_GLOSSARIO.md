...
Interface HTTP que permite chamar o Veltrix. Exemplo: `POST /api/orchestrate`.

## Backend

Parte do sistema que roda no servidor. No Veltrix, o backend e FastAPI em Python.
...
Recuperacao por busca/embeddings para anexar contexto. Nao existe no Veltrix atual.

## Fine-tuning

Treinar ou ajustar pesos de um modelo. O Veltrix nao faz isso.

## Treinamento

Processo de criar ou modificar um modelo de IA. Fora do escopo do Veltrix.
...
Decisao assistida sobre avancar ou bloquear release. No Veltrix, so confia em `local_qa` com evidencia textual limpa.
...
Significa que o Veltrix aceita requests com `origin_system=finguard`, mas nao le nem altera o repositorio FinGuard.
