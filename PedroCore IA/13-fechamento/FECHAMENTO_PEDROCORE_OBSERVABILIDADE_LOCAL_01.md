...
**Observabilidade visual técnica do PedroCore implementada e testada localmente.** A frente instrumenta o pipeline real, preserva o safe mode e não transforma memória/avaliação em alegação de treinamento.
...
- `3b11b36` — CORS local, falha total QA e replay FinGuard → PedroCore.
...
- tentativa principal, resultado, fallback, motivo sanitizado e duração ficam visíveis no PedroCore;
...
- execução real FinGuard → PedroCore → FinGuard com mock local;
- relatório QA sanitizado analisado e ingerido na memória técnica volátil;
- fallback controlado visível no PedroCore e oculto tecnicamente no FinGuard;
- falha total diagnosticada no PedroCore e erro amigável no FinGuard;
