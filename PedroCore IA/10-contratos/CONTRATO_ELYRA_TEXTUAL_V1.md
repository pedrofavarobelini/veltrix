...
Este contrato é a fronteira oficial e mínima entre Elyra e PedroCore para a
...
`X-PedroCore-Api-Key`. O payload não escolhe identidade. Missing, unknown,
...
O consumer comum nunca envia `provider=gemini` nem `model`. O PedroCore pode
...
- PedroCore não acessa PostgreSQL, Supabase, Storage ou filesystem da Elyra.
