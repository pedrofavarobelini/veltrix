...
- task types permitidos: `qa_report_analysis`, `qa_failure_diagnosis`, `release_gate_review`, `exploratory_test_plan`, `manual_exploration_report`, `assisted_exploration_review`, `artifact_summary`, `technical_explanation`;
...
O cliente HTTP no repositório do FinGuard (montar payload, enviar com a API key, tratar `blocked_reason`/`release_gate`) será implementado em frente separada, no repositório do FinGuard, com aprovação própria. Esta fase não acessa, não lê e não altera o FinGuard real.
