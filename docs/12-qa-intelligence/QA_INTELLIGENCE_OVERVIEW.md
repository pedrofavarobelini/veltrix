...
QA Intelligence é uma camada futura do PedroCore IA para:
...
- O PedroCore deve aceitar conteúdo Markdown enviado no payload (`artifacts[].content`), como já especificado em `docs/10-contratos/CONTRATO_ORQUESTRACAO.md`.
- O PedroCore deve extrair informações por **interpretação textual** (o provider de IA interpretando o texto), não por um parser rígido de seções fixas.
- O PedroCore **não deve depender inicialmente de JSON estruturado** vindo do FinGuard.
...
| `next_commands_suggestion` | Sugerir próximos comandos de investigação a partir de uma falha | `message` + `artifacts` (log, relatório) | Lista de `suggested_commands` (texto, nunca executado) | Recomendado | Sim, para teste | Não crítico isoladamente | PedroCore nunca executa os comandos sugeridos |
...
- **`suggested_commands`** — lista de comandos que poderiam ser executados por um humano ou pelo QA Automation do FinGuard para investigar mais. **Nunca são executados pelo PedroCore.**
- **`suggested_fixes`** — lista de sugestões textuais de correção. **Nunca são aplicadas automaticamente pelo PedroCore.**
...
- **`confidence`** — grau de confiança do PedroCore na análise (0 a 1), refletindo qualidade dos artefatos recebidos e se houve fallback.
...
- `suggested_fixes` **não são correções aplicadas** — são apenas texto sugerido; nenhuma mudança de código ou configuração é feita pelo PedroCore.
...
**O PedroCore não decide sozinho.** `can_advance` é sempre uma recomendação. **O PedroCore recomenda; o usuário/desenvolvedor aprova.** Nenhuma ação de release, merge ou deploy é disparada automaticamente a partir dessa resposta.
...
- Mexer em banco de dados (do PedroCore ou de qualquer sistema externo).
- Rodar Prisma.
- Rodar migrations.
- Rodar seed/reset.
