...
**AI Runtime & Learning Control Plane**

[![License: Apache 2.0](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](LICENSE)

Licenciado sob a **Apache License 2.0** (SPDX: `Apache-2.0`). Veja [LICENSE](LICENSE).

> **Renomeado de PedroCore para Veltrix.** O nome do produto mudou; os
> identificadores técnicos foram preservados de propósito. Contratos
> (`pedrocore-integration/v1`), tabelas (`pedrocore_*`), variáveis
...
elas falam com o Veltrix. Ele resolve **identidade, autorização, provider,
modelo e prompt**, e devolve uma resposta padronizada.

```text
FinGuard  ─┐
Structa   ─┤
Projeto C ─┼──→  Veltrix  ──→  Providers (Gemini, …)
Projeto N ─┘         │
...
- Reformulação documental `PEDROCORE-REPLAN-01` (01A a 01E) concluída: visão oficial, contratos técnicos, arquitetura-alvo, QA Intelligence e fechamento (ver `Veltrix/13-fechamento/FECHAMENTO_PEDROCORE_REPLAN_01.md`).
...
- **`POST /api/orchestrate`** existe: API operacional para sistemas externos (warnings com severidade, `warning_codes`, `error_code`, `blocked_reason`, `qa`, `release_gate`, `audit` completo), com autenticação interna opcional (`PEDROCORE_INTERNAL_API_KEY` + header `X-Veltrix-Api-Key`).
...
- **Contrato FinGuard → Veltrix** (Bloco 8) definido por payload fake: `origin_system` `finguard`/`finguard-local` read-only, sem qualquer acesso ao repositório real (ver `Veltrix/11-integracoes/CONTRATO_FINGUARD_PEDROCORE.md`).
...
- **Consumer Elyra textual V1**: `origin_system=elyra`, credencial registrada
  ...
  permanecem negados; Veltrix não acessa banco/Storage Elyra nem recalcula
  métricas oficiais.
...
determinístico preparado por ela para interpretação não clínica. O Veltrix
...
- `Veltrix/10-contratos/CONTRATO_ELYRA_TEXTUAL_V1.md`
- `Veltrix/17-multi-provider-safe-evolution/PEDROCORE_ELYRA_ONBOARDING_V1_TEXTUAL.md`
- `Veltrix/17-multi-provider-safe-evolution/GATE_PEDROCORE_ELYRA_ONBOARDING_V1_TEXTUAL.md`
- `Veltrix/MOC_QA_SAFETY_HARDENING.md`
- `Veltrix/MOC_ESTUDO_PEDROCORE.md`
- `Veltrix/00-visao-geral/README.md`
- `Veltrix/00-visao-geral/OBJETIVO.md`
- `Veltrix/03-versoes/ROADMAP.md`
- `Veltrix/09_STATUS_ATUAL.md`
- `Veltrix/07-decisoes/DECISOES_TECNICAS.md`
- `Veltrix/08_CHANGELOG.md`
...
| Estado | Significado |
| --- | --- |
| **Visível** | O Veltrix conhece a IA; ela aparece nas Configurações e no seletor |
...
- **Ditado por voz** (Web Speech API): o áudio não é gravado, guardado nem
  enviado ao Veltrix; só o texto transcrito entra no campo, para revisão.
...
adiada formalmente para a V2 — ver `Veltrix/20-ux-v1/V2_MULTIMODAL.md`.
...
- **FinGuard** — consumidor homologado. Usa `provider=auto` sem escolher
  modelo; o Veltrix resolve identidade, autorização, provider e modelo. O
  FinGuard não recebe nem repassa chaves. O Veltrix não lê, escreve nem
...
Detalhamento por cenário: `Veltrix/20-ux-v1/MODELO_DE_AMEACA.md`.
...
- **Cancelamento remoto**: após timeout, o Veltrix fecha a conexão local, mas
...
- Testes: `Veltrix/MOC_TESTES.md`
- Integrações: `Veltrix/MOC_INTEGRACOES.md`
- Status atual: `Veltrix/09_STATUS_ATUAL.md`
